...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] ShipNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is ShipNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (ShipNetworkObject.Length > 0 && ShipNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(ShipNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<ShipBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateShip instead, its shorter and easier to type out ;)")]
		public ShipBehavior InstantiateShipNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ShipNetworkObject[index]);
			var netBehavior = go.GetComponent<ShipBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<ShipBehavior>().networkObject = (ShipNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of Ship
		/// </summary>
		/// <returns>
		/// A local instance of ShipBehavior
		/// </returns>
		/// <param name="index">The index of the Ship prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public ShipBehavior InstantiateShip(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ShipNetworkObject[index]);
			var netBehavior = go.GetComponent<ShipBehavior>();
